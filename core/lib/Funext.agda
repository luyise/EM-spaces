...
{-
Consequences of the weak function extensionality axiom
-}
...
module _ {j} {P : A → Type j} (e : (x : A) → is-contr (P x)) where
...
module StrongFunextDep {j} {P : A → Type j} where
  ...
  app=-is-equiv = is-eq app= λ= (λ h → λ= (app=-β h)) (! ∘ λ=-η)

  λ=-is-equiv : (f g : Π A P)
    → is-equiv (λ= {f = f} {g = g})
  λ=-is-equiv f g = is-eq λ= app= (! ∘ λ=-η) (λ h → λ= (app=-β h))
...
module _ where
  ...
  private
    module λ=-is-equiv {j} {P : A → Type j} {f g : Π A P}
      = is-equiv (StrongFunextDep.λ=-is-equiv f g)
  ...
  λ=-equiv : ∀ {j} {P : A → Type j} {f g : Π A P}
    → (f ∼ g) ≃ (f == g)
  λ=-equiv {f = f} {g = g} = (λ= , λ=-is-equiv) where
    abstract
      λ=-is-equiv : is-equiv (λ= {f = f} {g = g})
      λ=-is-equiv = StrongFunextDep.λ=-is-equiv f g
