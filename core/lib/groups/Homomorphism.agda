...
preserves-comp-prop Ac Bc =
  preserves-comp Ac Bc , λ f → (Π-level (λ a₁ → (Π-level (λ a₂ → ⟨⟩))))
...
abstract
  ...
  group-hom=-β : ∀ {i j} {G : Group i} {H : Group j} {φ ψ : G →ᴳ H}
    (eq : GroupHom.f φ == GroupHom.f ψ) → ap (GroupHom.f {G = G} {H = H}) (group-hom= {G = G} {H = H} {φ = φ} {ψ = ψ} eq) == eq
  group-hom=-β {G = G} {H = H} {φ = φ} {ψ = ψ} idp = prop-path (has-level-apply (Π-level λ _ → ⟨⟩) _ _) (ap (GroupHom.f) (group-hom= {G = G} {H = H} idp)) idp
  ...
  instance
    ...
    GroupHom-level {G = G} {H = H} = equiv-preserves-level
      (equiv (uncurry group-hom) (λ x → GroupHom.f x , GroupHom.pres-comp x)
             (λ _ → idp) (λ _ → idp))
      {{Subtype-level
        (preserves-comp-prop (Group.comp G) (Group.comp H)) {{Π-level λ EG → ⟨⟩}}}}
