...
instance
  ...
  Π-level {A=A} {B=B} {n = ⟨-2⟩} p = has-level-in ((λ x → contr-center (p x)) , lemma p )
    where abstract
      lemma : ∀ {i j} {A : Type i} {B : A → Type j} (p : (x : A) → has-level ⟨-2⟩ (B x)) (f : Π A B) → (λ x → contr-center (p x)) == f
      lemma p f = λ= (λ x → contr-path (p x) (f x))
  Π-level {n = S n} p = has-level-in (lemma p) 
    where abstract
      lemma : ∀ {i j} {A : Type i} {B : A → Type j} {n : ℕ₋₂} (p : (x : A) → has-level (S n) (B x)) (f g : Π A B) → has-level n (f == g)
      lemma p f g =
