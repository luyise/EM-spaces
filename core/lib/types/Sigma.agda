...
instance
  ...
  Σ-level {n = ⟨-2⟩} {A = A} {P = P} p q = has-level-in ((contr-center p , (contr-center (q (contr-center p)))) , λ s → lemma)
    where abstract 
      lemma : {s : Σ A P} →
        contr-center p , contr-center (q (contr-center p)) == s
      lemma {s = s} = pair= (contr-path p (fst s)) (from-transp! _ _ (contr-path (q _) _))
      
  Σ-level {n = S n} {A = A} {P = P} p q = has-level-in (λ s t → lemma) where
    abstract
      lemma : {s t : Σ A P} → has-level n (s == t)
      lemma {s=s} {t=t} = equiv-preserves-level (=Σ-econv _ _)
